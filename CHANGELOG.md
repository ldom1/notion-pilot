...
### Added
- Cockpit header uses the same page/approval mark as the landing nav.
- Cockpit Telegram card: circled-i control explaining capture + CRM commands (not in the public docs yet).
- Cockpit uses the same dotted-graph scene as the landing page. Panels stay solid so counts stay readable; header is frosted.
...
### Fixed
- Deploy wizard success: **Open in Notion** was invisible — `.lp a { color: inherit }` beat `.btn-primary` color. Scoped under `.lp .lp-setup-actions a.btn-primary`.
- Deploy wizard log: removed the capped scroll panel; log grows in place with tighter lines. Success screen drops the log for a short summary.
- Deploy wizard on `?connected=1`: the CRM page name defaulted to empty while the tree leaf still showed "My awesome CRM", so **Deploy the CRM** stayed disabled with no explanation. Prefills the name, dims the disabled button, and shows why.

### Changed
- Cockpit Telegram info control: the button is the ring (no nested SVG circle); heavier filled i, hover wash.
- Landing page uses the same dotted-graph scene as the `?connected=1` wizard (fixed wallpaper; tinted bands are translucent so it shows through).
- Landing nav: Sign in (top right, same `/auth/notion?next=/cockpit` as the footer).
- Landing primary CTA (`.lp-btn-lg`): a bit wider via fluid padding + `min-width`, full-width wrap under 460px.
- Landing `#film` headline: "Nobody opened Notion…" implied the workspace was unused. It now reads "Open Notion to see the pipeline. It already moved." — Notion stays the CRM you open; the typing is what no longer requires a trip into it.
- Landing: dropped the film figcaption and the "film above is this path" line under the assistant surface. Footer is now a full-bleed bar (brand + GitHub + sign-in), and the page/approval mark is the favicon as well as the nav logo — replacing the old purple compass.
