...
1. **Notion MCP** authenticated to the Artelys CRM workspace, preferred. If missing after a genuine fresh-session check (not `/resume`, not a nested/wrapped session — confirm before concluding MCP is actually unreachable): fall back to `notion_pilot`'s own `Settings()` + `notion_client.AsyncClient` directly (same underlying client `NotionCompanySyncer`/`NotionPeopleSyncer` already use in `syncer.py` — not a new write path, just invoked in-process instead of via MCP RPC). Two things to check before writing this way: (a) load `Settings()` under the Infisical env that actually holds a valid Notion token for this workspace — `dev` may hold a placeholder/invalid token, verify with a cheap read call first; (b) the Companies data source may be on the legacy `data_sources` API rather than standard `databases` — probe with `data_sources.retrieve`/`databases.retrieve` and use whichever succeeds, since idempotent-property-creation helpers like `ensure_siren_property` in `syncer.py` only cover the standard `databases` path. Only stop and ask the user to connect MCP if this direct fallback is also not viable (e.g. no valid token anywhere).
2. **Any Notion write** must follow **`notion-crm-ops` write discipline** (French preview table, explicit `go`, never invent LinkedIn/SIREN/financials/dirigeants, prefer `notion-crm` tools then Notion MCP fallback, then the direct-API fallback above). Read that skill for tooling preference; do not reinvent write logic here.
...
4. Ground field shapes in `notion_pilot_powers/mcp/models.py` (`CompanyRecord`), `notion_pilot_powers/core/siren_lookup.py`, `notion_pilot_powers/core/prosper_client.py`, and `skills/notion-crm-ops/references/companies.md` + `crm-ids.md`.
...
- Map to Notion: `section_activite_principale`/`activite_principale` → `Sector` via `naf_section_to_sector`; `tranche_effectif_salarie` → `Size` via `tranche_to_size` (both in `notion_pilot_powers/core/siren_lookup.py`).
- This mirrors `notion_pilot_powers/core/siren_lookup.py::lookup_siren_candidates` exactly — same URL, same params.
