...
ACTIVITIES_ID = os.environ.get(
    "NOTION_ACTIVITIES_DATABASE_ID", "38f6c451-9465-8166-a862-e531d15f467f"
)
...
async def create_activity(client: httpx.AsyncClient, token: str, meeting: dict) -> str:
    ...
    if notes_text:
        activity["properties"]["Notes"] = {"rich_text": [{"text": {"content": notes_text[:2000]}}]}
    ...
    if r.status_code != 200:
        logger.error(
            "Failed to create Activity for '{}': {} {}", meeting_name, r.status_code, r.text
        )
        r.raise_for_status()
    return r.json()["id"]
