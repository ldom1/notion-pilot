...
async def enrich_people(limit: int, dry_run: bool, report_dir: Path) -> None:
    ...
    for candidate in candidates[:limit]:
        ...
        if not props:
            ...
            report.rows.append(PeopleRow(name, company, candidate["page_id"], enrichment.source or "", status="no_data"))
            continue
        ...
        report.rows.append(PeopleRow(
            name, company, candidate["page_id"], enrichment.source or "",
            email=enrichment.email, seniority=enrichment.seniority,
        ))
    ...
    logger.info(
        ...
        report.wrote, report.skipped_no_data,
    )
...
async def enrich_companies(limit: int, dry_run: bool, report_dir: Path) -> None:
    ...
    report = RunReport(table="Companies", dry_run=dry_run, total_in_db=len(company_syncer._id_to_name))
    ...
    for page_id, name in items[:limit]:
        ...
        logger.info(
            ...
            enrichment.country or "-", enrichment.sector or "-",
            "yes" if set_icon else "no", enrichment.source,
        )

        if dry_run:
            logger.info("  [DRY-RUN] would write {} props{} to {}",
                        len(props), " + icon" if set_icon else "", page_id)
        else:
            update_kwargs: dict[str, object] = {"properties": props}
            if set_icon:
                update_kwargs["icon"] = {"type": "external", "external": {"url": enrichment.logo_url}}
            await client.pages.update(page_id, **update_kwargs)
            await asyncio.sleep(_RATE_LIMIT_S)
            report.wrote += 1

        report.rows.append(CompanyRow(
            name, page_id, enrichment.source or "",
...
def main() -> None:
    ...
    parser.add_argument("--limit", type=int, default=9999, help="Max records to process (default: all)")
