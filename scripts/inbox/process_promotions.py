...
Usage:
    uv sync --extra email
    uv run python scripts/inbox/process_promotions.py --dry-run
    uv run python scripts/inbox/process_promotions.py
    uv run python scripts/inbox/process_promotions.py --from-csv   # rows marked Treated and archived
    uv run python scripts/inbox/process_promotions.py --dry-run --since-days=14
    uv run python scripts/inbox/process_promotions.py --limit=10   # newest 10 only (smoke test)
"""
...
async def _archive_uids(
    adapter: EmailAdapter, folder: str, uids: list[int], *, label: str
) -> bool:
    if not uids:
        return True
    try:
