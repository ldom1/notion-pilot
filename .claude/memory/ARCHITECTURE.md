...
- **CRM home linked views use a separate Notion API version** (2026-09-13, PR #30). CRUD/bootstrap stays on `2022-06-28`; Views calls go through `shared/notion_views.py` with `NOTION_VIEWS_VERSION` (default `2025-09-03`). Create order: empty page → append `crm_home_blocks()` → five DBs + seed → `create_home_views` under the **This week** heading (fail-soft warnings + optional manual callout). On `2022-06-28` list children, linked views appear as `child_database` with title `Untitled` — do not discover source DBs by those titles. Canonical doc URLs: `shared/doc_links.py` (cockpit `SourcesPanel.tsx` must stay in parity).

## Agent skills (project)

- `skills/notion-crm-ops/` — Artelys CRM create/update via Notion MCP (+ optional `notion-crm` stdio). Symlinked to `.cursor/skills/` and `.claude/skills/`. Always preview-gated.
